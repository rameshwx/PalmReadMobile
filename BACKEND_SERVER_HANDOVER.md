...
- Project root on server: `<deploy-path>`
...
- API base: `http://<vps-host>:8080`
- API base path (mobile app): `http://<vps-host>:8080/palmread/api`
- API health: `http://<vps-host>:8080/palmread/api/health`
- Admin dashboard: `http://<vps-host>:8080/palmread/admin`
- App health: `http://<vps-host>:8080/up`
- CV health: `http://<vps-host>:8001/health`

## 5) Project File Locations

### Root

- Repo root: `<deploy-path>`
- Root compose file: `<deploy-path>/docker-compose.yml`
- Root env file: `<deploy-path>/.env`
- Root README: `<deploy-path>/README.md`

### Laravel API

- Laravel project: `<deploy-path>/laravel_api`
- Laravel env: `<deploy-path>/laravel_api/.env`
- Routes: `<deploy-path>/laravel_api/routes/api.php`
- Queue job: `<deploy-path>/laravel_api/app/Jobs/ProcessPalmReadJob.php`
- Reading logic: `<deploy-path>/laravel_api/app/Services/Reading`
- API controllers: `<deploy-path>/laravel_api/app/Http/Controllers/Api`
- Local image storage (inside repo bind): `<deploy-path>/laravel_api/storage/app/palms`

### CV service

- CV project: `<deploy-path>/cv_service`
- Entry point: `<deploy-path>/cv_service/app/main.py`
- Pipeline modules: `<deploy-path>/cv_service/app/pipeline`
- Python deps: `<deploy-path>/cv_service/requirements.txt`

### Flutter app

- Flutter project: `<deploy-path>/flutter_app`
- App config: `<deploy-path>/flutter_app/lib/core/config/app_config.dart`
...
From `<deploy-path>/laravel_api/.env` (key items):
...
- Host path: `<deploy-path>/laravel_api/storage/app/firebase/service-account.json`
...
Note: As of 2026-02-16, the live VPS folder `<deploy-path>` may not be a git checkout (no `.git/`).
